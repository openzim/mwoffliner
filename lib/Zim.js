...
Zim.prototype.createDirectories = function (cb) {
    ...
    async_1.default.series([
        ...
        Utils_js_1.default.exitIfError(error, "Unable to create mandatory directories : " + error);
        cb();
    });
};
...
Zim.prototype.createSubDirectories = function (cb) {
    var env = this.env;
    var dirs = this.config.output.dirs;
...
Zim.prototype.executeTransparently = function (command, args, callback, nostdout, nostderr) {
    ...
    try {
        var proc = child_process_1.spawn(command, args).on('error', function (error) {
            Utils_js_1.default.exitIfError(error, 'Error in executeTransparently(), ' + error);
        });
        if (!nostdout) {
    ...
    if (!env.nozim) {
        child_process_1.exec('sync', function () {
            ...
            zim.executeTransparently('zimwriterfs', [
                ...
                Utils_js_1.default.exitIfError(error, 'Failed to build successfuly the ZIM file ' + zimPath + ' (' + error + ')');
