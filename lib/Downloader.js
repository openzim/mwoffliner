...
Downloader.prototype.downloadContent = function (url, callback, var1, var2, var3) {
    ...
    async_1.default.retry(3, function (finished) {
        var request;
        ...
        request = (protocol).get(options, function (response) {
            if (response.statusCode == 200) {
                ...
                response.on('end', function () {
                    ...
                    if (encoding == 'gzip') {
                        zlib_1.default.gunzip(Buffer.concat(chunks), function (error, decoded) {
                            callFinished(0, error, decoded && decoded.toString());
                        });
                    }
                    else if (encoding == 'deflate') {
                        zlib_1.default.inflate(Buffer.concat(chunks), function (error, decoded) {
                            callFinished(0, error, decoded && decoded.toString());
                        });
                    }
                    else {
...
Downloader.prototype.downloadMediaFile = function (url, path, force, optQueue, callback) {
    ...
    var self = this;
    fs_1.default.stat(path, function (error) {
        if (error && !force) {
            Utils_1.default.exitIfError(error.code !== 'ENOENT' && error, "Impossible to stat() " + path + ":");
            ...
            self.downloadContent(url, function (content, responseHeaders) {
                fs_1.default.writeFile(path, content, function (error) {
                    Utils_1.default.exitIfError(error, "Unable to write " + path + " (" + url + ")");
