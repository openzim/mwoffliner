  ...
  execute: function(argv) {
    ...
    function getArticleIds(finished) {
      ...
      function getArticleIdsForNamespaces() {
        async.eachLimit(mw.namespacesToMirror, mw.namespacesToMirror.length, getArticleIdsForNamespace, function(
          error
        ) {
          if (error) {
    ...
    function saveFavicon(finished) {
      ...
      if (customZimFavicon) {
        ...
        downloader.downloadContent(apiUrl + 'action=query&meta=siteinfo&format=json', function(
          content,
          responseHeaders
        ) {
          var body = content.toString();
