...
const { OfflinerEnv } = require('./OfflinerEnv.js');
...
function execute(argv) {
  ...
  function saveArticles(dump, finished) {
    ...
    function storeDependencies(parsoidDoc, articleId, finished) {
      ...
      const downloadAndSaveModule = (module, type) => {
        ...
        const hackStartUpModule = jsCode => jsCode.replace(
    ...
    function applyOtherTreatments(parsoidDoc, articleId, finished) {
      ...
      filtersConfig.cssClassBlackList.map((classname) => {
        nodesToDelete.push({ class: classname });
      });

      if (env.nodet) {
        filtersConfig.nodetCssClassBlackList.map((classname) => {
          nodesToDelete.push({ class: classname });
        });
      }

      /* Remove element with black listed CSS classes and no link */
      filtersConfig.cssClassBlackListIfNoLink.map((classname) => {
        nodesToDelete.push({
          class: classname,
          filter(n) {
      ...
      filtersConfig.idBlackList.map((id) => {
  ...
  function downloadFileAndCache(url, callback) {
    ...
    redis.getMedia(filenameBase, (error, r_width) => {
      /* If no redis entry */
      if (error || !r_width || r_width < width) {
        ...
        redis.saveMedia(filenameBase, width, () => {
          ...
          const cachePath = `${zim.cacheDirectory
            }m/${
  ...
  function getMainPage(finished) {
    ...
    function createMainPage(finished) {
      ...
      let html = '<ul>\n';
      Object.keys(articleIds).sort().map((articleId) => {
