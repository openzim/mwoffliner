...
function execute(argv) {
  ...
  const leadSectionTemplate = swig.compile(readTemplate(config.output.templates.lead_section));
  ...
  function getArticleIds(finished) {
    ...
    function getArticleIdsForLine(line, finished) {
      if (line) {
        ...
        setTimeout(f, redirectQueue.length() > 30000 ? redirectQueue.length() - 30000 : 0, url, function(content) {
          const body = content.toString();
  ...
  function saveFavicon(finished) {
    ...
    if (customZimFavicon) {
      ...
      downloader.downloadContent(`${apiUrl}action=query&meta=siteinfo&format=json`, function(content) {
        const body = content.toString();
