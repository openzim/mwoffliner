...
module.exports = {
    ...
    execute: function (argv) {
        ...
        /* Compile templates */
        var redirectTemplate = swig.compile(fs.readFileSync(config.output.templates.redirects, 'utf-8'));
        var footerTemplate = swig.compile(fs.readFileSync(config.output.templates.footer, 'utf-8'));
        ...
        // this module has no css, blacklisting it avoids creating an empty file that generate an error in firefox
        ...
        var htmlMobileTemplateCode =  fs.readFileSync(config.output.templates.mobile, 'utf-8').replace('__CSS_LINKS__', cssLinks);
        var htmlDesktopTemplateCode = fs.readFileSync(config.output.templates.desktop, 'utf-8');
        /* Get content */
        async.series(
            [
