...
module.exports = {
    ...
    execute: function (argv) {
        ...
        var downloader = new Downloader(mw,
        ...
        function login(finished) {
            if (mw.username && mw.password) {
                var url = apiUrl + 'action=login&format=json&lgname=' + mw.username + '&lgpassword=' + mw.password;
                if (mw.domain != '') {
                    url = url + '&lgdomain=' + mw.domain;
                }

                downloader.downloadContent(url, function (content, responseHeaders) {
                    var body = content.toString();
                    var jsonResponse = JSON.parse(body)['login'];
                    downloader.cookie = jsonResponse['cookieprefix'] + '_session=' + jsonResponse['sessionid'];
        ...
        function saveArticles(dump, finished) {
            ...
            function saveArticle(articleId, finished) {
                ...
                if (mobileLayout && mainPageId !== articleId) {
                    ...
                    printLog( 'Getting (mobile) article from ' + articleApiUrl );
                    fetch(articleApiUrl, {
                        method: 'GET',
                        headers: { 'Accept': 'application/json' }
                    })
                    ...
                    printLog( 'Getting (desktop) article from ' + articleUrl );
                    setTimeout(skipHtmlCache || articleId == mainPageId ? downloader.downloadContent : downloadContentAndCache, downloadFileQueue.length() + optimizationQueue.length(), articleUrl, function(content, responseHeaders, articleId) {
                        let json;
