...
require("./jsutils"); // we should avoid monkey-patching
...
function getParametersList() {
    return parameterList_1.default; // Want to remove this anonymous function. Need to investigate to see if it's needed
}
function execute(argv) {
    ...
    Utils_1.default.exitIfError(argv.speed && isNaN(argv.speed), 'speed is not a number, please give a number value to --speed');
    ...
    var downloader = new Downloader_1.default(logger, mw, config_1.default.userAgent + ' (' + adminEmail + ')', argv.requestTimeout || config_1.default.defaults.requestTimeout);
    /*
    ...
    var zimOpts = {
        ...
        creator: creator,
        // ZIM publisher
        publisher: argv.publisher || config_1.default.defaults.publisher,
        langIso2: 'en',
        langIso3: 'eng',
        ...
        cacheDirectory: (argv.cacheDirectory ? argv.cacheDirectory : path_1.default.resolve(process.cwd(), 'cac')) + '/',
        // Layout
    ...
    var env = new OfflinerEnv_1.default(argv.format, {
        zim: zim,
        mw: mw,
        logger: logger,
    ...
    try {
        ...
        (function () { }); // empty function because catches require
    }
    ...
    var readTemplate = function (t) {
        return fs_1.default.readFileSync(path_1.default.resolve(__dirname, t), 'utf-8');
    };
    ...
    async_1.default.series([
        ...
        function (finished) {
            async_1.default.eachSeries(env.dumps, function (dump, finished) {
                ...
                async_1.default.series([
                    function (finished) { return zim.createSubDirectories(finished); },
                    function (finished) { return zim.mobileLayout ? saveStaticFiles(finished) : finished(); },
                    function (finished) { return saveStylesheet(finished); },
                    function (finished) { return saveFavicon(finished); },
                    function (finished) { return getMainPage(finished); },
                    function (finished) { return env.writeHtmlRedirects ? saveHtmlRedirects(finished) : finished(); },
                    function (finished) { return saveArticles(dump, finished); },
                    function (finished) { return drainDownloadFileQueue(finished); },
                    function (finished) { return drainOptimizationQueue(finished); },
                    function (finished) { return zim.buildZIM(finished); },
                    function (finished) { return redis.delMediaDB(finished); }
                ], function (error, result) { return finished(); });
            }, function (error) {
                async_1.default.series([
                    function (finished) {
                        if (skipCacheCleaning) {
                            logger.log('Skipping cache cleaning...');
                            child_process_1.exec('rm -f "' + zim.cacheDirectory + 'ref"', finished);
                        }
                        else {
                            logger.log('Cleaning cache');
                            child_process_1.exec('find "' +
                                zim.cacheDirectory +
                                '" -type f -not -newer "' +
                                zim.cacheDirectory +
                                'ref" -exec rm {} \\;', finished);
                        }
                    }
                ], function (error, result) { return finished(); });
            });
    ...
    function saveStaticFiles(finished) {
        config_1.default.output.cssResources.forEach(function (css) {
            try {
                fs_1.default.readFile(path_1.default.resolve(process.cwd(), 'res', css + '.css'), function (err, data) {
                    return fs_1.default.writeFile(path_1.default.resolve(env.htmlRootPath, cssPath(css)), data, function () { });
                });
            }
            catch (error) {
    ...
    function saveArticles(dump, finished) {
        ...
        function storeDependencies(parsoidDoc, articleId, finished) {
            ...
            node_fetch_1.default(articleApiUrl, {
                ...
                styleDependenciesList = styleDependenciesList.filter(function (oneStyleDep) { return !config_1.default.filters.blackListCssModules.includes(oneStyleDep); });
            ...
            var downloadAndSaveModule = function (module, type) {
                ...
                var hackStartUpModule = function (jsCode) {
                ...
                var hackMediaWikiModule = function (jsCode) {
                ...
                redis.saveModuleIfNotExists(dump, module, moduleUri, type)
                    ...
                    redisResult === 1 &&
                        node_fetch_1.default(moduleApiUrl, {
                            method: 'GET',
        ...
        function treatMedias(parsoidDoc, articleId, finished) {
            ...
            for (var i = 0; i < imageNodes.length; i++) {
                ...
                var image = imageNode.getElementsByTagName('img').length
                            ...
                            if (params) {
                                ...
                                if (semiPieces && semiPieces.length === 2) {
                                    ...
                                    var deg = function (hemiHash) {
                                        var out = 0;
                                        var hemiSign = 0;
                                        for (var i = 0; i < 4 && i + offs_1 < pieces_1.length; i++) {
        ...
        function applyOtherTreatments(parsoidDoc, articleId, finished) {
            ...
            var emptyChildFilter = function (n) {
                return !n.innerHTML;
            };
    ...
    function getArticleIds(finished) {
        ...
        function parseAPIResponse(body) {
            ...
            if (entries) {
                var redirectQueueValues_1 = new Array();
                var details_1 = new Object();
        ...
        function getArticleIdsForNamespace(namespace, finished) {
            var next = '';
            async_1.default.doWhilst(function (finished) {
                logger.log("Getting article ids for namespace \"" + namespace + "\" " +
    ...
    function downloadContentAndCache(url, callback, var1, var2, var3) {
        ...
        async_1.default.series([
            ...
            if (error) {
                ...
                Utils_1.default.touch(cachePath, cacheHeadersPath);
        ...
        redis.getMedia(filenameBase, function (error, r_width) {
            ...
            if (error || !r_width || r_width < width) {
                ...
                redis.saveMedia(filenameBase, width, function () {
                    ...
                    var cachePath = zim.cacheDirectory +
                        'm/' +
                        crypto_1.default.createHash('sha1').update(filenameBase).digest('hex').substr(0, 20) +
                        (path_1.default.extname(url_1.default.parse(url, false, true).pathname || '') || '');
                    var cacheHeadersPath = cachePath + '.h';
    ...
    function getMediaBase(url, escape) {
        ...
        if (utf8_binary_cutter_1.default.getBinarySize(filename) > 249) {
            ...
            filename =
                utf8_binary_cutter_1.default.truncateToBinarySize(basename, 239 - ext.length) +
                    crypto_1.default.createHash('md5').update(basename).digest('hex').substring(0, 2) +
                    '.' +
                    ext;
        }
