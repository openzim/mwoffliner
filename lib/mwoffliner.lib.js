...
function execute(argv) {
  ...
  if (!parsoidUrl) {
    if (localParsoid) {
      ...
      parsoid
        ...
        .catch(err => {
          U.exitIfError(true, `Error starting Parsoid: ${err}`);
        });
          ...
          if (rel) {
            ...
            if (rel.substring(0, 10) === 'mw:ExtLink' || rel === 'nofollow') {
              ...
              if (isMirrored(targetId)) {
                ...
                redis.processRedirectIfExists(targetId, function(res) {
                  if (res) {
  ...
  function downloadFile(url, path, force, callback) {
    fs.stat(path, function(error) {
      if (error && !force) {
        U.exitIfError(error.code !== 'ENOENT', `Impossible to stat() ${path}: ${error}`);
