...
function execute(argv) {
  ...
  var env = new OfflinerEnv(argv.format, {
    zim: zim,
    mw: mw,
    logger: logger,
  ...
  var genHeaderScript = function(js) {
    return '<script src="' + jsPath(js) + '.js"></script>';
  };
      ...
      function rewriteUrl(linkNode, finished) {
        ...
        if (!href) {
          ...
          if (rel) {
            ...
            var targetId = extractTargetIdFromHref(href); // targetId already defined
            if (targetId) {
              if (isMirrored(targetId)) {
                linkNode.setAttribute('href', env.getArticleUrl(targetId));
                setImmediate(function() {
    ...
    function saveArticle(articleId, finished) {
      ...
      function buildArticleFromApiData() {
        if (html) {
          ...
          var prepareAndSaveArticle = async.compose(
            writeArticle,
            setFooter,
  ...
  function isSubpage(id) {
    if (id && id.indexOf('/') >= 0) {
      ...
      var namespace = mw.namespaces[namespace]; // namespace already defined
      if (namespace != undefined) {
  ...
  function getMediaBase(url, escape) {
    ...
    var filename = U.decodeURIComponent(
