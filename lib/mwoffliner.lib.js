...
module.exports = {
    ...
    execute: function (argv) {
        ...
        var zimConfig = {
            langIso2: 'en',
            langIso3: 'eng',
            articleList: argv.articleList, // List of articles is maybe in a file
        ...
        var articleIds = {};
        var namespaces = {};
        var webUrl = mw.base + mw.wikiPath + '/';
        ...
        async.series(
            [
                ...
                function (finished) {
                    mw.getTextDirection(downloader, function(ret) {
                        ltr = ret; // Update global state (FIXME)
                        finished();
                    });
                },
                function (finished) { mw.getSiteInfo(downloader, zimConfig, finished) },
                ...
                function (finished) {
                    async.eachSeries(
                        ...
                        function (dump, finished) {
                            ...
                            nodet = dump.toString().search('nodet') >= 0 ? true : false;
        ...
        function createDirectories(finished) {
            logger.log('Creating base directories...');
            async.series(
                [
        ...
        function computeFilenameRadical(withoutSelection, withoutContentSpecifier, withoutDate) {
            ...
            if (filenamePrefix) {
                ...
                var langSuffix = zimConfig.langIso2;
                for (var i = 0; i < hostParts.length; i++) {
                    if (hostParts[i] === zimConfig.langIso3) {
            ...
            if (!withoutSelection) {
                if (articleList) {
                    radical += '_' + pathParser.basename(articleList, pathParser.extname(articleList)).toLowerCase().replace(/ /g, mw.spaceDelimiter);
                } else {
                    radical += '_all';
                }
            }
            ...
            if (!withoutContentSpecifier) {
                radical += nopic ? '_nopic' : novid ? nodet ? '' : '_novid' : '';
                radical += nodet ? '_nodet' : '';
            }
        ...
        function buildZIM(finished) {
            if (!nozim) {
                ...
                // Compute tags
                var tags = nopic ? 'nopic' : novid ? '=novid' : '';
                tags = tags + (nodet ? tags ? ';nodet' : 'nodet' : '');

                exec('sync', function (error) {
                    var zimPath = computeZimRootPath();
                    var cmd = 'zimwriterfs --welcome=index.htm --favicon=favicon.png --language=' + zimConfig.langIso3
                        + (zimConfig.mainPageId ? ' --welcome=' + getArticleBase(zimConfig.mainPageId) : ' --welcome=index.htm')
                        ...
                        + ' --title="' + zimConfig.name + '" --description="' + (zimConfig.description || subTitle || zimConfig.name) + '" --creator="' + creator + '" --publisher="'
                        + publisher + '" "' + htmlRootPath + '" "' + zimPath + '"';
                    logger.log('Building ZIM file ' + zimPath + ' (' + cmd + ')...');
                    logger.log('RAID: ' + computeZimName());
                    executeTransparently(
                        ...
                        [
                            ...
                            zimConfig.mainPageId ? '--welcome=' + getArticleBase(zimConfig.mainPageId) : '--welcome=index.htm',
                            '--favicon=favicon.png',
                            '--language=' + zimConfig.langIso3,
                            '--title=' + zimConfig.name,
                            '--name=' + computeZimName(),
                            '--description=' + (zimConfig.description || subTitle || zimConfig.name),
                            '--creator=' + creator,
                            '--publisher=' + publisher,
                            htmlRootPath,
                            zimPath
        ...
        function saveArticles(dump, finished) {
            ...
            function applyOtherTreatments(parsoidDoc, articleId, finished) {
                ...
                nodesToDelete.push({
                    class: 'gallerybox',
                    filter: function(n) {
                        return nopic || !n.getElementsByClassName('thumb').length;
                    }
                });
            ...
            function setFooter(parsoidDoc, articleId, finished) {
                ...
                if (mobileLayout) {
                    ...
                    if (zimConfig.mainPageId != articleId) {
                ...
                /* Subpage */
                if (isSubpage(articleId) && zimConfig.mainPageId != articleId) {
            ...
            function saveArticle(articleId, finished) {
                let html = '';
                if (mobileLayout && zimConfig.mainPageId !== articleId) {
                    ...
                    setTimeout(skipHtmlCache || articleId == zimConfig.mainPageId ? downloader.downloadContent : downloadContentAndCache, downloadFileQueue.length() + optimizationQueue.length(), articleUrl, function(content, responseHeaders, articleId) {
                        let json;
        ...
        function isMirrored(id) {
            if (!articleList && id && id.indexOf(':') >= 0) {
                var namespace = namespaces[id.substring(0, id.indexOf(':')).replace(/ /g, mw.spaceDelimiter)];
                    ...
                    try {
                        if (!JSON.parse(body)['error']) {
                            ...
                            JSON.parse(body)['query']['backlinks'].map(function (entry) {
                                ...
                                if (title == zimConfig.mainPageId) {
                                    zimConfig.mainPageId = articleId;
                                }
                            });
        ...
        function getArticleIds(finished) {
            ...
            async.series(
                [
                    function (finished) { getArticleIdsForLine(zimConfig.mainPageId, finished) },
                    function (finished) {
                        if (articleList) {
                    ...
                    function (finished) {
                        if (articleList) {
                            ...
                            if (!isMirrored(zimConfig.mainPageId)) {
                                getArticleIdsForLine(zimConfig.mainPageId, finished)
                            } else {
                                finished();
                            }
                        }
        ...
        function createSubDirectories(finished) {
            logger.log('Creating sub directories at \"' + htmlRootPath + '\"...');
            async.series(
                [
        ...
        function getArticleBase(articleId, escape) {
            var filename = articleId.replace(/\//g, mw.spaceDelimiter);
            var dirBase = filename.replace(/\./g, mw.spaceDelimiter);
        ...
        function getSubTitle(finished) {
            ...
            downloader.downloadContent(webUrl, function (content, responseHeaders) {
                var html = content.toString();
                var doc = domino.createDocument(html);
        ...
        function getMainPage(finished) {
            ...
            function createMainPageRedirect(finished) {
                ...
                var html = redirectTemplate({
                    title: zimConfig.mainPageId.replace(/_/g, ' '),
                    target: getArticleBase(zimConfig.mainPageId, true)
                });
                writeMainPage(html, finished);
            }

            if (zimConfig.mainPageId) {
        ...
        function getNamespaces(finished) {
            ...
            downloader.downloadContent(url, function (content, responseHeaders) {
                ...
                types.map(function (type) {
                    ...
                    Object.keys(entries).map(function (key) {
                        var entry = entries[key];
                        var name = entry['*'].replace(/ /g, mw.spaceDelimiter);
                        var number = entry['id'];
                        var allowedSubpages = ('subpages' in entry);
                        var isContent = entry['content'] != undefined || addNamespaces.contains(number) ? true : false;
                        var canonical = entry['canonical'] ? entry['canonical'].replace(/ /g, mw.spaceDelimiter) : '';
                        var details = { 'number': number, 'allowedSubpages': allowedSubpages, 'isContent': isContent };

                        /* Namespaces in local language */
                        namespaces[U.lcFirst(name)] = details;
                        namespaces[U.ucFirst(name)] = details;

                        /* Namespaces in English (if available) */
                        if (canonical) {
                            namespaces[U.lcFirst(canonical)] = details;
                            namespaces[U.ucFirst(canonical)] = details;
                        }
