...
function main({
  ...
  optBinaries.forEach(cmd => {
    const execDone = (error, stdout, stderr) => {
      if (error) {
        console.error(`Failed to find binary "${cmd.split(" ")[0]}": (${error})`);
        process.exit(1);
      }
    };
    exec(cmd, execDone, true, true);
  });
  ...
  /* Some helpers */
  const readTemplate = t => fs.readFileSync(pathParser.resolve(__dirname, t), "utf-8");
  const dirs = config.output.dirs;
  const cssPath = css => [dirs.style, dirs.styleModules, css].join("/");
  const jsPath = js => [dirs.javascript, dirs.jsModules, js].join("/");
  const genHeaderCSSLink = css => `<link href="${cssPath(css)}" rel="stylesheet" type="text/css" />`;
  const genHeaderScript = js => `<script src="${jsPath(js)}"></script>`;
  ...
  const saveStaticFiles = finished => {
    // const dirs = config.output.dirs;
  ...
  const extractTargetIdFromHref = href => {
    try {
  ...
  const drainOptimizationQueue = finished => {
  ...
  const saveHtmlRedirects = finished => {
  ...
  const saveArticles = (dump, finished) => {
      ...
      fetch(articleApiUrl, {
        ...
        .then(({ parse: { modules, modulescripts, modulestyles, headhtml } }) => {
          ...
          for (let i = 0; i < scriptTags.length; i++) {
            if (scriptTags[i].text.includes("mw.config.set")) jsConfiglets = regex.exec(scriptTags[i].text);
          }
          ...
          try {
            fs.writeFileSync(pathParser.resolve(env.htmlRootPath, jsPath("jsConfiglets.js")), jsConfiglets);
    ...
    function setFooter(parsoidDoc, articleId, finished) {
      const htmlTemplateDoc = domino.createDocument(
        (zim.mobileLayout ? htmlMobileTemplateCode : htmlDesktopTemplateCode)
          .replace("__ARTICLE_CONFIGletS_LIST__", jsConfiglets !== "" ? genHeaderScript("jsConfiglets") : "")
          .replace(
          "__ARTICLE_JS_LIST__",
  ...
  const addNoIndexCommentToElement = element => {
    const slices = element.parentElement.innerHTML.split(element.outerHTML);
    element.parentElement.innerHTML =
      slices[0] + "<!--htdig_noindex-->" + element.outerHTML + "<!--/htdig_noindex-->" + slices[1];
  }
  ...
  const getArticleIds = finished => {
          ...
          if ("missing" in entry) {
            ...
            if (entry.revisions) {
              ...
              const articleDetails = {
                t: parseInt(new Date(entry.revisions[0].timestamp).getTime() / 1000)
              };
    ...
    function getArticleIdsForLine(line, finished) {
      if (line) {
        ...
        const url =
          apiUrl + "action=query&redirects&format=json&prop=revisions|coordinates&titles=" + encodeURIComponent(title);
  ...
  const createSubDirectories = finished => {
  ...
  const downloadContentAndCache = (url, callback, let1, let2, let3) => {
        ...
        function (finished) {
          fs.readFile(cacheHeadersPath, function (error, data) {
            try {
              ...
              finished(
                "Error in downloadContentAndCache() JSON parsing of " + cacheHeadersPath + ", error is: " + error
              );
            }
          });
        }
  ...
  const downloadFileAndCache = (url, callback) => {
          ...
          if (error) {
            ...
            if (fs.existsSync(cacheHeadersPath) && fs.existsSync(cachePath)) {
              ...
              try {
                ...
                console.error(
                  "Error in downloadFileAndCache() JSON parsing of " + cacheHeadersPath + ", error is: " + error
                );
  ...
  const downloadFile = (url, path, force, callback) => {
  ...
  const getMediaBase = (url, escape) => {
    ...
    const filenameSecondletiant = parts[5] + (parts[6] || ".svg") + (parts[7] || "");
    let filename = U.decodeURIComponent(
      filenameFirstletiant.length > filenameSecondletiant.length ? filenameFirstletiant : filenameSecondletiant
    );
  ...
  const optimizationQueue = async.queue((file, finished) => {
    ...
    fs.stat(path, function (error, stats) {
      if (!error && stats.size === file.size) {
        ...
        const retryTask = finished => {
          exec(cmd, (executionError, stdout, stderr) => {
            if (!executionError) finished();
            fs.stat(path, function (error, stats) {
              if (!error && stats.size > file.size) finished(null, true);
              else if (!error && stats.size < file.size) finished("File to optim is smaller (before optim) than it should.");
              else exec(`file -b --mime-type "${path}"`, function (error, stdout, stderr) {
                const type = stdout.replace(/image\//, "").replace(/[\n\r]/g, "");
                cmd = getOptimizationCommand(path, type);
                if (cmd) setTimeout(finished, 2000, executionError);
                else finished("Unable to find optimization command.");
              });
            });
          });
        };
        const retryTaskDone = (error, skip) => {
          if (error) console.error(`Executing command : ${cmd}\nFailed to optim ${path}, with size=${file.size} (${error})`);
          else if (skip) logger.log(`Optimization skipped for ${path}, with size=${file.size}, a better version was downloaded meanwhile.`);
          else logger.log(`Successfuly optimized ${path}`);
          finished();
        };
        async.retry({ times: 5 }, retryTask, retryTaskDone);
      } else {
      ...
      finished => {
        async.eachSeries(
          ...
          error => {
            ...
            async.series(cacheCleaning, finished);
          }
        );
      }
    ],
    error => {
      async.series(
        ...
        error => {
          ...
          process.exit(0); /* Time to time the script hungs here. Forcing the exit */
        }
      );
    }
  );
  ...
  const downloadingFileQueueTask = (url, finished) => {
    if (url) downloadFileAndCache(url, finished);
    else finished();
  };
  const downloadFileQueue = async.queue(downloadingFileQueueTask, speed * 5);
}
