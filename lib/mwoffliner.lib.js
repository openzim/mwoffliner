    ...
    execute: function (argv) {
        ...
        async.series(
            [
                ...
                function (finished) { env.checkResume(finished) },
                        ...
                        function (dump, finished) {
                            ...
                            async.series(
                                [
                                    ...
                                    function (finished) { zim.mobileLayout ? saveStaticFiles(finished) : finished() },
                                    ...
                                    function (finished) { zim.buildZIM(finished) },
        ...
        function saveArticles(dump, finished) {
                ...
                for (var i = 0; i < imageNodes.length; i++) {
                    ...
                    if (!env.nopic && imageNode && image) {
                        ...
                        if (imageNodeTypeof.indexOf('mw:Image/Thumb') >= 0 ||
                            imageNodeTypeof.indexOf('mw:Video/Thumb') >= 0 || zim.mobileLayout) {
            ...
            function saveArticle(articleId, finished) {
                ...
                if (zim.mobileLayout && zim.mainPageId !== articleId) {
                    ...
                    setTimeout(skipHtmlCache || articleId == zim.mainPageId ? downloader.downloadContent.bind(downloader) : downloadContentAndCache, downloadFileQueue.length() + optimizationQueue.length(), articleUrl, function(content, responseHeaders, articleId) {
                        let json;
        ...
        function getMainPage(finished) {
            ...
            function createMainPage(finished) {
                ...
                var doc = domino.createDocument(
                    (zim.mobileLayout ? htmlMobileTemplateCode : htmlDesktopTemplateCode).replace('__ARTICLE_JS_LIST__', '').replace('__ARTICLE_CSS_LIST__', '').replace('__ARTICLE_CONFIGVARS_LIST__', '')
                );
