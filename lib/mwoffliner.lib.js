            ...
            return new Promise(function (resolve, reject) {
                ...
                function saveArticle(articleId, finished) {
                    ...
                    if (zim.mobileLayout && zim.mainPageId !== articleId) {
                        ...
                        var articleUrl_1 = parsoidUrl
                            + encodeURIComponent(articleId)
                            + (parsoidUrl.indexOf('/rest') < 0 ? (parsoidUrl.indexOf('?') < 0 ? '?' : '&') + "oldid=" : '/')
                            + articleIds[articleId];
                        logger.log("Getting (desktop) article from " + articleUrl_1);
                        var downloadFunc_1 = skipHtmlCache || articleId === zim.mainPageId
                        ...
                        setTimeout(function (url, handler) {
                            downloadFunc_1(url)
                                ...
                                .catch(function (err) {
                                console.error("Failed to get article [" + articleUrl_1 + "] Skipping.", err);
                                finished();
                            });
                        }, downloadFileQueue.length() + optimizationQueue.length(), articleUrl_1, function (content) {
                            var json;
