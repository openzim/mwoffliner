...
module.exports = {
    ...
    execute: function (argv) {
        ...
        var zim = new Zim(zimOpts, logger);
        ...
        async.series(
            [
                ...
                function (finished) { zim.prepareCache(env, finished) },
                ...
                function (finished) {
                    async.eachSeries(
                        ...
                        function (dump, finished) {
                            ...
                            keepHtml = env.nozim ? true : keepHtml;
        ...
        function computeZimRootPath() {
            var zimRootPath = zim.outputDirectory[0] === '/' ? zim.outputDirectory : pathParser.resolve(process.cwd(), zim.outputDirectory) + '/';
            zimRootPath += env.computeFilenameRadical() + '.zim';
            return zimRootPath;
        }

        function computeZimName() {
            return (publisher ? publisher.toLowerCase() + '.' : '') + env.computeFilenameRadical(false, true, true);
        }
            ...
            if (!env.nozim) {
                ...
                exec('sync', function (error) {
                    ...
                    var cmd = 'zimwriterfs --welcome=index.htm --favicon=favicon.png --language=' + zim.langIso3
                        + (zim.mainPageId ? ' --welcome=' + getArticleBase(zim.mainPageId) : ' --welcome=index.htm')
                        + (deflateTmpHtml ? ' --inflateHtml ' : '')
                        + (verbose ? ' --verbose ' : '')
                        + (tags ? ' --tags="' + tags +'"' : '')
                        + ' --name="' + computeZimName() + '"'
                        + (withZimFullTextIndex ? ' --withFullTextIndex' : '')
                        + (writeHtmlRedirects ? '' : ' --redirects="' + zim.redirectsCacheFile + '"')
                        + ' --title="' + zim.name + '" --description="' + (zim.description || subTitle || zim.name) + '" --creator="' + zim.creator + '" --publisher="'
                    ...
                    executeTransparently(
                        ...
                        [
                            ...
                            '--title=' + zim.name,
                            '--name=' + computeZimName(),
                            '--description=' + (zim.description || subTitle || zim.name),
                            '--creator=' + zim.creator,
                            '--publisher=' + publisher,
                            htmlRootPath,
                            zimPath
        ...
        function getMainPage(finished) {
            ...
            function createMainPageRedirect(finished) {
                ...
                var html = redirectTemplate({
                    ...
                    target: getArticleBase(zim.mainPageId, true)
                });
