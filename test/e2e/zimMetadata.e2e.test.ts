    ...
    test(`check all zim metadata using zimdump for ${outFiles[0]?.renderer} renderer`, async () => {
      ...
      await Promise.all(
        [
          ...
          expect(await zimdump(`show --ns M --url "${option}" ${outFiles[0].outFile}`)).toBe(output)
        }),
      )

      expect(await zimdump(`show --ns M --url "Illustration_48x48@1" ${outFiles[0].outFile}`)).toBeDefined()
    })
