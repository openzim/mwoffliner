...
test('Simple articleList', async (t) => {
    ...
    for (const dump of outFiles) {
        if (dump.nopic) {
            t.ok(dump.status.files.success > 30 && dump.status.files.success < 700, 'nopic has enough files');
