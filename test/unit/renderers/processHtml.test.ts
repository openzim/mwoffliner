...
describe('processHtml', () => {
  ...
  describe('iframe', () => {
    ...
    it('replace full non-youtube iframe with external placeholder', async () => {
      ...
      const link = finalDoc.querySelector('#foo .mwo-external-video-link')
    ...
    it('replace multiple iframes', async () => {
     ...
     it('rewrites youtube.com embed iframe', async () => {
      ...
      expect(finalDoc.querySelector('#foo iframe') == null).toBe(true)
      expect(finalDoc.querySelector('#foo .mwo-external-video-card')).not.toBeNull()
      expect(finalDoc.querySelector('#foo .mwo-external-video-link')?.getAttribute('href')).toBe('https://www.youtube.com/watch?v=VIDEO_ID')
      expect(finalDoc.querySelector('#foo .mwo-external-video-link')?.getAttribute('class')).toContain('external')
      expect(finalDoc.querySelector('#foo .mwo-external-video-thumbnail img')?.getAttribute('src')).toContain('_assets_')
      // No overlay
    ...
    it('preserves iframe attributes on youtube placeholder card', async () => {
      const { finalHTML } = await testProcessHtml(
        `<div id="foo"><iframe class="yt-player" width="560" height="315" src="https://www.youtube.com/embed/ATTR_ID"></iframe></div>`,
      )
      const finalDoc = domino.createDocument(finalHTML)
      const card = finalDoc.querySelector('#foo .mwo-external-video-card')
      expect(card).not.toBeNull()
      expect(card?.getAttribute('class')).toContain('yt-player')
      expect(card?.getAttribute('width')).toBe('560')
      expect(card?.getAttribute('height')).toBe('315')
    })
    ...
    it('rewrites youtube-nocookie embed iframe', async () => {
      ...
      const finalDoc = domino.createDocument(finalHTML)
      expect(finalDoc.querySelector('#foo iframe') == null).toBe(true)
      expect(finalDoc.querySelector('#foo .mwo-external-video-link')?.getAttribute('href')).toBe('https://www.youtube.com/watch?v=NOC_ID')
    })
    ...
    it('rewrites youtu.be iframe', async () => {
      ...
      const finalDoc = domino.createDocument(finalHTML)
      expect(finalDoc.querySelector('#foo iframe') == null).toBe(true)
      expect(finalDoc.querySelector('#foo .mwo-external-video-link')?.getAttribute('href')).toBe('https://www.youtube.com/watch?v=SHORT_ID')
    })
    ...
    it('rewrites youtube watch iframe', async () => {
      ...
      const finalDoc = domino.createDocument(finalHTML)
      expect(finalDoc.querySelector('#foo iframe') == null).toBe(true)
      expect(finalDoc.querySelector('#foo .mwo-external-video-link')?.getAttribute('href')).toBe('https://www.youtube.com/watch?v=WATCH_ID')
    })

    it('falls back to generic text link when youtube id extraction fails', async () => {
      const { finalHTML } = await testProcessHtml(`<div id="foo"><iframe src="https://www.youtube.com/watch?feature=share"></iframe></div>`)
      const finalDoc = domino.createDocument(finalHTML)
      const link = finalDoc.querySelector('#foo .mwo-external-video-link')
      expect(finalDoc.querySelector('#foo iframe') == null).toBe(true)
      expect(finalDoc.querySelector('#foo .mwo-external-video-fallback')).not.toBeNull()
      expect(finalDoc.querySelector('#foo .mwo-external-video-thumbnail')).toBeUndefined()
