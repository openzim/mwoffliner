...
`
  ...
  describe('treatMedias', () => {
    ...
    test('treatMedias format="nopic" preserves meaningful alt text in placeholder', async () => {
      const { dump } = await setupScrapeClasses({ format: 'nopic' }) // en wikipedia
      ...
      expect(svg).toContain('Lecture demonstrating conservation of mechanical energy')
      expect(ret.imageDependencies).toHaveLength(0)
    })
    ...
    test('treatMedias format="nopic" uses 1x1 fallback when dimensions are missing', async () => {
      ...
      expect(svg).toContain('No dimensions')
      expect(ret.imageDependencies).toHaveLength(0)
    })
  })
})
