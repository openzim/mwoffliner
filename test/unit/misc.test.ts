  ...
  describe('getStringsForLang', () => {
    ...
    test('returns en strings if lang file is missing completely', () => {
      ...
      expect(strings).toEqual({
        ...
        DOWNLOAD_ERRORS_LINE1_DELETED_ARTICLE: "This article was deleted after we compiled the list of articles to retrieve but before we fetched it to build the ZIM you are browsing.",
        DOWNLOAD_ERRORS_LINE1_WIKIMEDIA_DESKTOP_API_HTML_500_ERROR: "When the ZIM you are browsing was built, ${server} server failed to render this article HTML content and returned an HTTP 500 error.",
        DOWNLOAD_ERRORS_LINE1_WIKIMEDIA_DESKTOP_API_HTML_504_UPSTREAM_TIMEOUT: "When the ZIM you are browsing was built, ${server} server timed-out while rendering this article HTML content and returned an HTTP 504 error.",
