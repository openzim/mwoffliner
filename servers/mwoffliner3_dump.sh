...
MWOFFLINER="$SCRIPT_DIR/mwoffliner.js --withZimFullTextIndex --adminEmail=contact@kiwix.org --deflateTmpHtml --verbose --skipHtmlCache --format= --format=nopic"
MWMATRIXOFFLINER="$SCRIPT_DIR/mwmatrixoffliner.js --speed=4 --verbose --skipHtmlCache --adminEmail=contact@kiwix.org --mwUrl=https://meta.wikimedia.org/ --deflateTmpHtml --skipCacheCleaning --withZimFullTextIndex"
...
cat "$SCRIPT_DIR/medicine" | /srv/kiwix-tools/tools/scripts/listLangLinks.pl --host=en.wikipedia.org --path=w --readFromStdin --language=as --language=bn --language=gu --language=hi --language=kn --language=ml --language=bpy --language=mr --language=or --language=pa --language=ta --language=te --language=ur --language=fa --language=fr --language=zh --language=pt --language=ar --language=es --language=it > "$SCRIPT_DIR/medicine.langlinks"
...
/srv/kiwix-tools/tools/scripts/listCategoryEntries.pl --host=en.wikipedia.org --category="Évaluation_des_articles_Soins_infirmiers_et_profession_infirmière" --category="Évaluation_des_articles_de_premiers_secours_et_de_secourisme" --category="Évaluation_des_articles_de_médecine" --category="Évaluation_des_articles_Anatomie" --category="Évaluation_des_articles_pharmacie" --path=w --exploration=5 --namespace=1 | sed 's/Talk://' | sort -u >> "$SCRIPT_DIR/medicine" &&
cat "$SCRIPT_DIR/medicine" | sort -u > "$SCRIPT_DIR/medicine_fr"
$MWOFFLINER --speed=5 --mwUrl="https://fr.wikipedia.org/" --parsoidUrl="https://fr.wikipedia.org/api/rest_v1/page/html/" --customZimTitle="Encyclopédie médicale WikiMed" --customMainPage="Utilisateur:Doc_James/Open_Textbook_of_Medicine" --customZimDescription="L'Encyclopédie médicale de Wikipédia" --customZimFavicon="$SCRIPT_DIR/medicine.png" --articleList="$SCRIPT_DIR/medicine_fr" --outputDirectory=$ZIM2INDEX/wikipedia/ &&
