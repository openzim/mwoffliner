...
export class RedisKvs<T> {
  ...
  public exists(prop: string | string[]): Promise<{ [key: string]: number } | number> {
  ...
  public iterateItems(numWorkers: number, func: (items: KVS<T>, runningWorkers: number) => Promise<void>) {
    return new Promise((resolve, reject) => {
  ...
  private mapKeysGet(obj: any): T {
    ...
    if (obj && this.keyMapping && typeof obj === 'object') {
      mappedVal = Object.entries(obj).reduce((acc, [key, val]) => {
