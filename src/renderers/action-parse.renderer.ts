...
export class ActionParseRenderer extends Renderer {
  ...
  public templateDesktopArticle(bodyCssClass: string, htmlCssClass: string, hideFirstHeading: boolean, moduleDependencies: RenderOptsModules, articleId: string): Document {
    ...
    const htmlTemplateString = this.#htmlTemplateCode()
      .replace('__ARTICLE_JAVASCRIPT__', Downloader.trustedJs ? javaScriptTemplateCode() : '')
      ...
      .replace('__ARTICLE_CONFIGVARS_LIST__', JSON.stringify(jsConfigVars))
      .replace('__ARTICLE_JS_LIST__', JSON.stringify(jsDependenciesList))
      .replace('__ARTICLE_CSS_STATE__', JSON.stringify(articleCssState))
