...
export abstract class Renderer {
  ...
  private convertImageToPlaceholder(img: DominoElement) {
    ...
    const svgPayload = this.getPlaceholderSvgPayload(width, height, meaningfulAlt)
  ...
  private getPlaceholderSvgPayload(width: number | null, height: number | null, meaningfulAlt: string | null) {
    ...
    if (meaningfulAlt) {
      ...
      text = `<text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle" fill="#54595d" font-size="${fontSize}" font-family="sans-serif">${this.escapeXml(meaningfulAlt)}</text>`
    }

    return `<svg xmlns="http://www.w3.org/2000/svg" width="${safeWidth}" height="${safeHeight}" viewBox="0 0 ${safeWidth} ${safeHeight}" preserveAspectRatio="none">${title}<rect width="100%" height="100%" fill="transparent"/>${text}</svg>`
  }
