...
import { interpolateTranslationString } from '../util/misc.js'
  ...
  for (const matchingRule of matchingRules) {
    if (
      ...
      (!matchingRule.rawResponseDataContains ||
        matchingRule.rawResponseDataContains.findIndex((rawResponseDataContain) => typeof err.responseData == 'string' && err.responseData.includes(rawResponseDataContain)) >= 0) &&
      (!matchingRule.jsonResponseDataContains ||
        matchingRule.jsonResponseDataContains.findIndex((jsonResponseDataContain) => typeof err.responseData == 'object' && 
          jsonMatch(err.responseData, jsonResponseDataContain.key, jsonResponseDataContain.valueContains),
        ) >= 0)
    ) {
      return matchingRule
    }
...
export function renderDownloadError(err: DownloadErrorContext, dump: Dump, articleTitle: string): string | null {
